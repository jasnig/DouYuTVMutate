...
import UIKit
private let commonRefreshTimeKey = "ZJCommonRefreshTimeKey"
...
extension RefreshViewDelegate {
    ...
    public var lastRefreshTime: NSDate? {
        get {
            return NSUserDefaults.standardUserDefaults().objectForKey(lastRefreshTimeKey ?? commonRefreshTimeKey) as? NSDate
            
        }
        set {
            NSUserDefaults.standardUserDefaults().setObject(newValue, forKey: lastRefreshTimeKey ?? commonRefreshTimeKey)
...
public class RefreshView: UIView {
    let LastRefreshTimeKey = NSProcessInfo().globallyUniqueString
    /// KVO Constant
...
extension RefreshView {
    ...
    private func adjustFooterWhenScrollViewIsScrolling(scrollView: UIScrollView) {
        ...
        // 已经进入拖拽状态, 进行相关操作
        /// 超出屏幕的内容高度
        let offPartHeight = scrollView.contentSize.height - heightOfContentOnScreenOfScrollView(scrollView)
        let beginAnimatingOffsetY = offPartHeight>=0 ? offPartHeight : -scrollViewOriginalValue.contentInset.top
